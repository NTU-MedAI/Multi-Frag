...
def Validation(validloader, model, metrics, opt,epoch,key):
    model.eval()
    All_answer = []
    All_label = []
    ...
    for ii, data in enumerate(validloader):
        ...
        if key==1 or epoch < 19:
            ...
            with open('jeshon_label.csv', 'a') as f:
                writer = csv.writer(f)
        ...
        output = model(Input,smiles,epoch,key)       #[batch, output_size]
        ...
        # print(output)
        for i in range(opt.args['TaskNum']):
            cur_task_output = output[:, i:(i+1)]    # [1, ClassNum]
            # cur_task_output = list(np.array(output).T[i])
